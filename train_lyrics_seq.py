...
class Rating_Prediction(object):
  ...
  def build_model(self):
    ...
    datasetv = datasetv.repeat(self.epoch*10000)
    ...
    self.pred_only_spec = self.lyrics_cnn(self.lyrics)
  ...
  def lyrics_cnn(self, inputs):
    regularizer = tf.contrib.layers.l2_regularizer(scale=0.1)

    with tf.variable_scope("only_spec"):
      print inputs.shape
      conv1 = tf.layers.conv1d(inputs, filters=256, kernel_size=5, kernel_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
                  activation=tf.nn.relu,kernel_regularizer=regularizer)

      # conv1 = tf.layers.conv2d(conv1, filters=10, kernel_size=(20, 10), kernel_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
      #             activation=tf.nn.relu)
      # conv1gmp = tf.reduce_max(conv1, reduction_indices=[1], name='gmp')
      ...
      # conv1p = tf.transpose(pool1, [0,1,3,2])
      print (conv1p.shape)

      conv2 = tf.layers.conv1d(conv1p, filters=128, kernel_size=5, kernel_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
              activation=tf.nn.relu,kernel_regularizer=regularizer)
      ...
      conv3 = tf.layers.conv1d(conv2p, filters=64, kernel_size=5, kernel_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
              activation=tf.nn.relu,kernel_regularizer=regularizer)
      conv3 = tf.contrib.layers.dropout(conv3, self.keep_prob_var)      
      conv3p = tf.layers.max_pooling1d(conv3, pool_size=2, strides=2, padding='SAME')
      print (conv3p.shape)

      # conv4 = tf.layers.conv1d(conv3p, filters=32, kernel_size=5, kernel_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
      #             activation=None,kernel_regularizer=regularizer)
      # conv4 = tf.contrib.layers.dropout(conv4, self.keep_prob_var)      
      # conv4p = tf.layers.max_pooling1d(conv4, pool_size=2, strides=2, padding='SAME')      
      # # conv4gmp = tf.reduce_max(conv4, reduction_indices=[1], name='gmp')
      
      flat = tf.layers.flatten(conv3p)
      fc = tf.layers.dense(flat, 512, name='fc1',kernel_regularizer=regularizer)
      # fc = tf.contrib.layers.dropout(fc, self.keep_prob_var)
      # fc = tf.nn.relu(fc)
    ...
    try:
      while not coord.should_stop():
        ...
        if (counter) % 2000 == 0 and (counter) > 0:
          self.saver.save(self.sess, "./only_lyrics"+str(counter)+"_th")
